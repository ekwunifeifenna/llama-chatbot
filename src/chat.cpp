    ...
    CROW_ROUTE(app, "/chat")
        .methods("POST"_method)([&](const crow::request &req)
            ...
            if(api_key != g_config.api_key) {
                ...
                auto res = crow::response(401, result);
            ...
            try {
                ...
                if (session.request_count++ > g_config.rate_limit) {
                    ...
                    auto res = crow::response(429, result);
                    res.add_header("Content-Type", "application/json");
                    return res;
                }

                std::string response_text = processor.process_message(message, session.history);
                size_t pos = response_text.find("</s>");
                if (pos != std::string::npos) response_text.erase(pos);

                crow::json::wvalue result{
                    {"text", response_text},
                    {"buttons", crow::json::wvalue::list()},
                    {"showOptions", true}
                };
